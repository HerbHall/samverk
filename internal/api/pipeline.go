...
func (a *API) handlePipelineStages(w http.ResponseWriter, r *http.Request) {
	...
	stageOrder := []string{store.StageOpen, store.StageQueued, store.StageClaimed, store.StageInProgress, store.StageNeedsQC, store.StageNeedsHuman, store.StageBlocked, store.StageDone, store.StageFailed}
