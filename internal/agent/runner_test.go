...
func TestBuildSystemPrompt(t *testing.T) {
	...
	prompt := buildSystemPrompt(task)

	if want := "code-gen agent"; !contains(prompt, want) {
		t.Errorf("prompt missing %q", want)
	}
	if want := "#99"; !contains(prompt, want) {
		t.Errorf("prompt missing %q", want)
	}
	if want := "Add dark mode"; !contains(prompt, want) {
		t.Errorf("prompt missing %q", want)
	}
	if want := "enhancement, ui"; !contains(prompt, want) {
		t.Errorf("prompt missing %q", want)
	}
}

func TestBuildSystemPromptNoLabels(t *testing.T) {
	task := Task{
		Issue: &forge.Issue{
			Number: 1,
			Title:  "Simple task",
	...
	if want := "research agent"; !contains(prompt, want) {
		t.Errorf("prompt missing %q", want)
	}
	if want := "Labels:"; contains(prompt, want) {
		t.Error("prompt should not contain Labels line when no labels")
	}
}
