...
func TestCheckOutputQuality(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "truncated output with continue marker",
			output:     strings.Repeat("x", 100) + " I'll continue from where I left off next time.",
			agentType:  models.AgentTypeResearch,
			wantPass:   false,
			wantScore:  0.3,
			wantReason: "output appears truncated",
		},
		{
			name:       "truncated output with let me continue",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := checkOutputQuality(tt.output, tt.agentType)
